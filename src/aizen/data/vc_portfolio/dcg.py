...
class DCGParser(BaseVCParser):
    def __init__(self, url: str = "https://dcg.co/portfolio", debug_mode=False):
    ...
    def parse_company_cards(self, content: str) -> list:
        ...
        cards = tree.xpath(
            '//div[starts-with(@class, "folio__item-content")]')
        ...
        for card in cards:
            try:
                ...
                name = card.xpath(
                    './/div[contains(@class, "folio__name")]//text()')
                name = name[0].strip() if name else ""

                description = card.xpath(
                    './/div[contains(@class, "portfolio__item-desc")]//text()')
                description = description[0].strip() if description else ""

                link_elements = card.xpath(
                    './/div[contains(@class, "folio__item-bottom")]//a')
                links = {}

                # Extract and print each link's href and text
                for link in link_elements:
                    link_url = link.get('href')
                    link_text = 'twitter' if 'twitter' in link_url else 'linkedin' if 'linkedin' in link_url else 'website'
                    if link_url and link_text:
                        links[link_text] = link_url

                logo_url = card.xpath(
                    './div[contains(@class, "folio__logo")]//img/@src')
