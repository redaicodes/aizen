...
class MulticoinParser(BaseVCParser):
    def __init__(self, url: str = "https://multicoin.capital/portfolio", debug_mode=False):
    ...
    def parse_company_cards(self, content: str) -> list:
        ...
        cards = tree.xpath(
            '//div[starts-with(@class, "Project__Wrapper-sc-")]')
        ...
        for card in cards:
            try:
                ...
                name = card.xpath(
                    './div[contains(@class, "Project__StyledName-sc-")]//text()')
                name = name[0].strip() if name else ""

                description = card.xpath(
                    './div[contains(@class, "Project__StyledDescription-sc-")]//text()')
                description = description[0].strip() if description else ""

                link_elements = card.xpath(
                    './div[contains(@class, "Project__StyledLinks-sc-")]//a')
                links = {}
                ...
                logo_url = card.xpath(
                    './div[contains(@class, "Project__StyledLogo-sc-")]//img/@src')
