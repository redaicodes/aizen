...
class ParadigmParser(BaseVCParser):
    def __init__(self, url: str = "https://paradigm.xyz/portfolio/", debug_mode=False):
    ...
    def parse_company_cards(self, content: str) -> list:
        ...
        cards = tree.xpath(
            '//div[contains(@class, "flex") and contains(@class, "flex-col") and contains(@class, "gap-1") and contains(@class, "relative")]')
        ...
        for card in cards:
            try:
                ...
                name = card.xpath(
                    './/div[contains(@class, "flex flex-col gap-half")]/p/a//text()')
                name = name[0].strip() if name else ""

                description = card.xpath(
                    './/div[contains(@class, "rich-text line-break font-mono text-mono-12")]/p//text()')
                description = description[0].strip() if description else ""

                link_elements = card.xpath(
                    './/div[contains(@class, "flex flex-col gap-half")]/p/a')
                links = {}
